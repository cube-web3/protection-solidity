...
contract ProtectionBase_Concrete_Unit_Test is BaseTest {
    ...
    function _deployProxyAndImplementation() internal {
        ...
        emit Cube3IntegrationDeployed(users.integrationAdmin, address(mockRouter), true);
        mockUpgradeableProxy = new ERC1967Proxy(
            address(mockUpgradeableImpl),
            abi.encodeCall(MockUpgradeable.initialize, (address(mockRouter), users.integrationAdmin, true))
        );
    ...
    function test_SucceedsWhen_DeployingProxyAndInitializingImplementation() public {
    ...
    function test_RevertsWhen_RouterAddressIsNull() public {
        ...
        mockUpgradeableProxy = new ERC1967Proxy(
            address(mockUpgradeableImpl),
            abi.encodeCall(MockUpgradeable.initialize, (address(0), users.integrationAdmin, true))
        );
    }
    ...
    function test_RevertsWhen_IntegrationAdminAddressIsNull() public {
        ...
        mockUpgradeableProxy = new ERC1967Proxy(
            address(mockUpgradeableImpl),
            abi.encodeCall(MockUpgradeable.initialize, (address(mockRouter), address(0), true))
        );
    }
    ...
    function test_RevertsWhen_PreRegistrationFails() public {
        ...
        mockUpgradeableProxy = new ERC1967Proxy(
            address(mockUpgradeableImpl),
            abi.encodeCall(MockUpgradeable.initialize, (address(mockRouter), users.integrationAdmin, false))
        );
    }
    ...
    function test_SucceedsWhen_UpgradingTheImplementation() public {
        ...
        mockUpgradeableProxy = new ERC1967Proxy(
            address(mockUpgradeableImpl),
            abi.encodeCall(MockUpgradeable.initialize, (address(mockRouter), users.integrationAdmin, connectToCube3ProtocolByDefault))
        );

        MockUpgradeable(address(mockUpgradeableProxy)).protectedFunction(stateArg1, new bytes(0));
        ...
        MockUpgradeable(address(mockUpgradeableProxy)).upgradeToAndCall(address(mockUpgradeableImplV2), new bytes(0));
        vm.stopPrank();

        MockUpgradeableV2(address(mockUpgradeableProxy)).protectedFunctionV2(stateArg2, new bytes(0));

        assertEq(MockUpgradeableV2(address(mockUpgradeableProxy)).state(), stateArg1);
        assertEq(MockUpgradeableV2(address(mockUpgradeableProxy)).stateV2(), stateArg2);
